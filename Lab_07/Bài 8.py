...
A = set(random.sample(danh_sach_tong, k=min(so_phan_tu_A, len(danh_sach_tong))))
so_nguyen = sum(1 for x in A if type(x) == int)
so_thuc = sum(1 for x in A if type(x) == float)
so_ky_tu = sum(1 for x in A if type(x) == str)
print(" Tập hợp A: ", A)
